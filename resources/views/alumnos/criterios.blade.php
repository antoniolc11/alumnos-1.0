...
@section('content')
    ...
    <div class="flex justify-center mt-10">
        <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
            <a href="#">
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Alumno {{$alumno->nombre}}</h5>
            </a>
            @if ($alumno->notas->isEmpty())
                <p>Todavia no existen notas para este alumno</p>
            @else
                @foreach ($alumno->notas as $nota)

                <p>
                    Criterio: {{ $nota->ccee->ce}} Nota: {{$nota->nota }}
                </p>
                @endforeach
            @endif
